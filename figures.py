...
def regress( times, vals, pdo=None, amo=None, enso=None, rindices=False, noautocorrelation=False ):
    ...
    If "enso" is set to amo as defined earlier in this notebook, then regression is also 
    performed against the AMO index timeseries "rindices". To do this, set "enso=enso". 

    By default, use unsmoothed indices of PDO and AMO if either (or both) is requested. 
    If you wish to use the indices convolved with a 5-month exponential on the other hand, 
    then specify rindices=True. 
    ...
    igood = np.argwhere( np.logical_not( np.ma.getmaskarray( vals ) ) ).squeeze()
    ...
    for ilabel, label in enumerate( labels ): 
        ...
        if label == "bias": 
            ...
            pvals = [] 
            for time in times[igood]:
                ii = np.argwhere( np.abs( enso['times'] - time ) < 0.5 / 12 )[0] 
                if rindices: 
                    pvals.append( enso['rindices'][ii] )
                else: 
                    pvals.append( enso['indices'][ii] )
            #  Remove the mean. 
            pvals = np.array( pvals ).flatten()
            fp[:,ilabel] = pvals - pvals.mean()
...
def expand( times, coeffs, covariance, labels, timereference, pdo=None, amo=None, enso=None, rindices=False ):
    ...
    fp = np.zeros( (times.size,len(labels)), dtype=np.float32 )
...
def get_pdo_timeseries(): 
    ...
    rf /= rf.sum()
    rindices = np.ma.convolve( indices, rf )[:indices.size]
...
def get_amo_timeseries( source="psl" ): 
    """Retrieve the latest index timeseries for the Atlantic Multidecadal Oscillation (AMO)
    and convolve it with an exponential response function with timescale of 5 months. If 
    the source is "psl", use the AMO index as published by the NOAA Physical Science 
    Laboratory (doi:10.1029/2000GL012745). If the source is "csu", use the AMO index as 
    generated by Colorado State (doi:10.1175/2008JCLI2162.1)."""
    ...
    if source=="csu": 
        ...
        amo_file = "psl_amo.csv"
        resp = requests.get( 'https://psl.noaa.gov/data/correlation/amon.us.long.data' )
    ...
    if source == "csu": 
        ...
        yearlimits = [ int(a) for a in re.split( r'\s+', lines[0].strip() ) ]
        nyears = yearlimits[1] - yearlimits[0] + 1
        lines = lines[1:nyears+1]
        years = []
        months = []
        indices = []
    ...
    x = np.array( indices )
    indices = np.ma.masked_where( np.abs(x) == 99.99, x )
    years = np.array( years )
    months = np.array( months )
...
def get_enso_timeseries(): 
    """Get ENSO index timeseries. The index is actually the Nino 3 SST index, derived from 
    hadISST1.1. We use this one because it is a measurement of a very broad swath of the 
    equatorial Pacific for SST. The data is obtained from the NOAA Physical Sciences 
    Laboratory. See doi:10.1029/2002JD002670."""

    print( 'get_enso_timeseries' )
    time0 = time()

    enso_file = "nino3.nc"
    resp = requests.get( 'https://psl.noaa.gov/data/timeseries/month/data/nino3.long.anom.nc' )
    with open( enso_file, 'wb' ) as f: 
        f.write( resp.content )

    #  Read data. 

    d = Dataset( enso_file, 'r' )
    data = d.variables['value'][:]

    v = d.variables['time']
    units = v.getncattr( "units" )
    m = re.search( r'^(\S+) since (\S+)', units )
    time_unit = m.group(1)
    ss = m.group(2).split("-")
    epoch = datetime( year=int(ss[0]), month=int(ss[1]), day=int(ss[2]) )
    vals = v[:]
    years, months = [], []
    for x in vals[:]: 
        date = epoch + timedelta( **{ time_unit: int(x) } )
    ...
    t_indices = data
    t_years = np.array( years )
    t_months = np.array( months )

    #  Reorganize into 12-month years. 

    fill_value = np.float32( -999.00 )
    nyears = t_years.max() - t_years.min() + 1
    indices = np.zeros( 12 * nyears, np.float32 ) + fill_value
    times = t_years.min() + ( np.arange( nyears*12 ) + 0.5 ) / 12
    years = np.int32( times )
    months = np.int32( 12 * times - 12 * years + 1 )

    for i in range( times.size ): 
        year, month = years[i], months[i]
        ii = np.argwhere( np.logical_and( t_years==year, t_months==month ) ).squeeze()
        if ii.size == 1: 
            indices[i] = t_indices[ii]

    indices = np.ma.masked_where( indices==fill_value, indices )
    ...
    enso = { 'years': years, 'months': months, 'indices': indices, 'rindices': rindices }
    enso['times'] = times
...
def get_berkeley_temperature(): 
    """Retrieve the Berkeley Earth temperature record."""

    print( 'get_berkeley_temperature' )
    time0 = time()

    fill_value = np.float32( -99.99 )
    berkeley_file = "berkeley_temperature.dat"

    resp = requests.get( "https://berkeley-earth-temperature.s3.us-west-1.amazonaws.com/Global/Land_and_Ocean_complete.txt" )

    with open( berkeley_file, 'wb' ) as f: 
        f.write( resp.content )

    with open( berkeley_file, 'r' ) as f: 
        lines = f.readlines()
    ...
    while len(lines) > 0: 
        kline = lines[0]
        lines = lines[1:]
        dataline = ( kline[0] != "%" and not re.search( r'^\s+$', kline ) )
    ...
    for line in lines: 
            ss = re.split( r'\s+', line.strip() )
            years.append( np.int32( ss[0] ) )
            months.append( np.int32( ss[1] ) )

            if ss[2] == "NaN": 
                monthly_data.append( fill_value )
            else: 
                monthly_data.append( np.float32( ss[2] ) )

            if ss[4] == "NaN": 
                annual_data.append( fill_value )
            else: 
                annual_data.append( np.float32( ss[4] ) )

    #  Convert to numpy arrays. 

    monthly_data = np.array( monthly_data, dtype=np.float32 )
    monthly_data = np.ma.masked_where( monthly_data == fill_value, monthly_data )
    annual_data = np.array( annual_data, dtype=np.float32 )
    annual_data = np.ma.masked_where( annual_data == fill_value, annual_data )
    years = np.array( years )
    months = np.array( months )

    ret = { 'years': years, 'months': months, 'monthly_data': monthly_data, 'annual_data': annual_data }
    return ret


def compute_ccmp_linear_regressions( outputdir, pdo=None, amo=None, enso=None, rindices=False ): 
    ...
    yearrange = [ np.int32( times.min() ), np.int32( times.max() ) ]

    print( '  CCMP methods = ' + ", ".join( methods ) )
    print( '  CCMP regions = ' + ", ".join( regions ) )

    #  PDO year range. 

    if pdo: 
        n = np.int32( pdo['rindices'].size / 12 )
        iyears = np.argwhere( np.logical_not( np.ma.getmaskarray( pdo['rindices'].reshape( n, 12 ) ).any( axis=1 ) ) ).squeeze()
        y1 = pdo['years'][::12]
        pdo_yearrange = [ y1[iyears[0]], y1[iyears[-1]] ]
        if pdo_yearrange[0] > yearrange[0]: 
            yearrange[0] = pdo_yearrange[0]
        if pdo_yearrange[1] < yearrange[1]: 
            yearrange[1] = pdo_yearrange[1]

    #  AMO year range. 

    if amo: 
        n = np.int32( amo['rindices'].size / 12 )
        iyears = np.argwhere( np.logical_not( np.ma.getmaskarray( amo['rindices'].reshape( n, 12 ) ).any( axis=1 ) ) ).squeeze()
        y1 = amo['years'][::12]
        amo_yearrange = [ y1[iyears[0]], y1[iyears[-1]] ]
        if amo_yearrange[0] > yearrange[0]: 
            yearrange[0] = amo_yearrange[0]
        if amo_yearrange[1] < yearrange[1]: 
            yearrange[1] = amo_yearrange[1]

    #  ENSO year range. 

    if enso: 
        n = np.int32( enso['rindices'].size / 12 )
        iyears = np.argwhere( np.logical_not( np.ma.getmaskarray( enso['rindices'].reshape( n, 12 ) ).any( axis=1 ) ) ).squeeze()
        y1 = enso['years'][::12]
        enso_yearrange = [ y1[iyears[0]], y1[iyears[-1]] ]
        if enso_yearrange[0] > yearrange[0]: 
            yearrange[0] = enso_yearrange[0]
        if enso_yearrange[1] < yearrange[1]: 
            yearrange[1] = enso_yearrange[1]

    print( f'  CCMP analysis yearrange = [ {yearrange[0]:4d}, {yearrange[1]:4d} ]' )

    #  Loop over CCMP analyses (method, region) of tropical width. 
    ...
    for imethod, method in enumerate( methods ):
        for iregion, region in enumerate( regions ):
            ...
            for imonth in range(12):
                ii = np.argwhere( np.logical_and( months == imonth+1, np.logical_and( years >= yearrange[0], years <= yearrange[1] ) ) ).squeeze() 
            ...
            for season in seasons:
                ...
                ii0, ii1 = ( months[ii] >= season['monthrange'][0] ), ( months[ii] <= season['monthrange'][1] )
                if season['monthrange'][1] > season['monthrange'][0]:
                    jj = np.argwhere( np.logical_and( ii0, ii1 ) ).squeeze()
                else:
                    jj = np.argwhere( np.logical_or( ii0, ii1 ) ).squeeze()
                y = dlats[ii[jj]]
                x = times[ii[jj]]

                #  Linear regression. 

                ret = regress( x, y, pdo=pdo, amo=amo, enso=enso, rindices=rindices, noautocorrelation=(season != "All") )
                ...
                rec = {
                    ...
                    'region': region,
                    'season': season['name'],
                    'lats': lats[ii[jj]],                 #  ...with annual cycle included...
                    'times': x,
                ...
                if enso: 
                    i = ret['labels'].index("enso")
                    rec.update( { 
                        'enso_index': i, 
                        'enso': ret['coefficients'][i],
                        'enso_uncertainty': np.sqrt( ret['uncertainty'][i,i] ) 
                    } )
    ...
    for a in ccmp_analyses: 
        ...
        if enso: 
            rec.update( {
                'enso': float( a['enso'] ),
                'enso_uncertainty': float( a['enso_uncertainty'] ) }  )

        out.append( rec )

    jsonsavefile = os.path.join( outputdir, "ccmp_analyses.json" )
    print( f'  Saving data to {jsonsavefile}' )
    with open( jsonsavefile, 'w' ) as f:
        json.dump( out, f, indent='  ' )

    #  Write trends and their uncertainties in user-friendly table format and LaTeX format. 

    nseasons = len( seasons )
    seasonnames = [ s['name'] for s in seasons ]
    ...
    for latex in [ False, True ]: 
        ...
        if latex: 
            ...
            lines.append( "Region" + " "*8 + "  ".join( [ f'{season:^14s}' for season in seasonnames ] ) )
        ...
        if latex: 
            outputfile = os.path.join( outputdir, "ccmp_trends.tex" )
        else: 
            outputfile = os.path.join( outputdir, "ccmp_trends.dat" )
    ...
    if pdo: 
        ...
        lines = [ " "*14 + '{:^80s}'.format( "Seasonal PDO (uncertainty) [degrees lat per PDO index]" ) ]
        lines.append( "Region" + " "*7 + "  ".join( [ f'{season["name"]:^14s}' for season in seasons ] ) )
        ...
        outputfile = os.path.join( outputdir, "ccmp_pdo.dat" )
        print( f'  Saving CCMP PDO estimates to {outputfile}.' )
        with open( outputfile, 'w' ) as f:
            f.write( "\n".join( lines ) + "\n" )
    ...
    if amo: 
        ...
        lines = [ " "*14 + '{:^80s}'.format( "Seasonal AMO (uncertainty) [degrees lat per AMO index]" ) ]
        lines.append( "Region" + " "*7 + "  ".join( [ f'{season["name"]:^14s}' for season in seasons ] ) )
        ...
        outputfile = os.path.join( outputdir, "ccmp_amo.dat" )
        print( f'  Saving CCMP AMO estimates to {outputfile}.' )
        with open( outputfile, 'w' ) as f:
            f.write( "\n".join( lines ) + "\n" )

    #  Write ENSO estimates and their uncertainties in user-friendly table format. 

    if enso: 

        lines = [ " "*14 + '{:^80s}'.format( "Seasonal ENSO (uncertainty) [degrees lat per ENSO index]" ) ]
        lines.append( "Region" + " "*7 + "  ".join( [ f'{season["name"]:^14s}' for season in seasons ] ) )
        ...
        for hemisphere in [ "N", "S" ]:
            for region in regions:
                ...
                for season in seasons:
                    rec = [ rec for rec in ccmp_analyses if rec['region']==region and
                        rec['season']==season['name'] and rec['method']=="All" ][0]
                    line += f'  {rec["enso"]:6.3f} ({rec["enso_uncertainty"]:5.3f})'
                lines.append( line )

        #  To an ASCII output file. 

        outputfile = os.path.join( outputdir, "ccmp_enso.dat" )
        print( f'  Saving CCMP ENSO estimates to {outputfile}.' )
        with open( outputfile, 'w' ) as f:
            f.write( "\n".join( lines ) + "\n" )
...
def compute_era5_linear_regression( outputdir, pdo=None, amo=None, enso=None, rindices=False ): 
    ...
    outputfile = os.path.join( outputdir, "era5_trends.dat" )
    ...
    if pdo: 
        ...
        outputfile = os.path.join( outputdir, "era5_pdo.dat" )
        print( f'  Saving ERA5 PDO estimates to {outputfile}.' )
        with open( outputfile, 'w' ) as f:
            f.write( "\n".join( lines ) + "\n" )
    ...
    if amo: 
        ...
        outputfile = os.path.join( outputdir, "era5_amo.dat" )
        print( f'  Saving ERA5 AMO estimates to {outputfile}.' )
        with open( outputfile, 'w' ) as f:
            f.write( "\n".join( lines ) + "\n" )
    ...
    if enso: 
        ...
        outputfile = os.path.join( outputdir, "era5_enso.dat" )
        print( f'  Saving ERA5 ENSO estimates to {outputfile}.' )
        with open( outputfile, 'w' ) as f:
            f.write( "\n".join( lines ) + "\n" )
...
def compute_cmip6_linear_regression( outputdir, pdo=None, amo=None, enso=None, rindices=False ): 
    ...
    print( f'  CMIP6 analysis yearrange = [ {yearrange[0]:4d}, {yearrange[1]:4d} ]' )
    ...
    for model in models: 
        ...
        for iregion, region in enumerate( regions ): 
            years, months = g.variables['years'][:], g.variables['months'][:]
            ii = np.argwhere( np.logical_and( years >= yearrange[0], years <= yearrange[1] ) ).squeeze() 
    ...
    jsonsavefile = os.path.join( outputdir, "cmip6_analyses.json" )
    print( f'  Saving data to {jsonsavefile}' )
    with open( jsonsavefile, 'w' ) as f: 
        json.dump( out, f, indent='  ' )
    ...
    for model in models: 
        ...
        lines.append( " "*14 + '{:^80s}'.format( "Seasonal trends (uncertainty) [deg/decade]" ) )
        lines.append( "Region" + " "*7 + "  ".join( [ f'{season["name"]:^14s}' for season in seasons ] ) )
    ...
    outputfile = os.path.join( outputdir, "cmip6_trends.dat" )
    print( f'  Saving CMIP6 trends estimates to {outputfile}.' )
    with open( outputfile, 'w' ) as f:
        f.write( "\n".join( lines ) + "\n" )
    ...
    if pdo: 
        ...
        for model in models: 
            lines.append( f'Model: {model}' )
            lines.append( " "*14 + '{:^80s}'.format( "Seasonal PDO (uncertainty) [degrees lat per PDO index]" ) )
            lines.append( "Region" + " "*7 + "  ".join( [ f'{season["name"]:^14s}' for season in seasons ] ) )
        ...
        outputfile = os.path.join( outputdir, "cmip6_pdo.dat" )
        print( f'  Saving CMIP6 PDO estimates to {outputfile}.' )
        with open( outputfile, 'w' ) as f:
            f.write( "\n".join( lines ) + "\n" )
    ...
    if amo: 
        ...
        for model in models: 
            lines.append( f'Model: {model}' )
            lines.append( " "*14 + '{:^80s}'.format( "Seasonal AMO (uncertainty) [degrees lat per AMO index]" ) )
            lines.append( "Region" + " "*7 + "  ".join( [ f'{season["name"]:^14s}' for season in seasons ] ) )
        ...
        outputfile = os.path.join( outputdir, "cmip6_amo.dat" )
        print( f'  Saving CMIP6 AMO estimates to {outputfile}.' )
        with open( outputfile, 'w' ) as f:
            f.write( "\n".join( lines ) + "\n" )
    ...
    if enso: 
        ...
        for model in models: 
            lines.append( f'Model: {model}' )
            lines.append( " "*14 + '{:^80s}'.format( "Seasonal ENSO (uncertainty) [degrees lat per ENSO index]" ) )
            lines.append( "Region" + " "*7 + "  ".join( [ f'{season["name"]:^14s}' for season in seasons ] ) )
        ...
        outputfile = os.path.join( outputdir, "cmip6_enso.dat" )
        print( f'  Saving CMIP6 ENSO estimates to {outputfile}.' )
        with open( outputfile, 'w' ) as f:
            f.write( "\n".join( lines ) + "\n" )
...
def compute_total_width_trend( ccmp_analyses, pdo=None, amo=None, enso=None, rindices=False ): 
    ...
    ret = regress( x, y, pdo=pdo, amo=amo, enso=enso, rindices=rindices )
...
def plot_amo_timeseries( amo, outputfile="amo_timeseries.pdf" ): 
    ...
    fig = plt.figure( figsize=(6,2) )
    ax = fig.add_axes( [0.08,0.20,0.91,0.76] )
    ...
    ax.plot( amo['times'], amo['indices'], color="#000000", lw=0.5, label="Unsmoothed AMO" )
    ax.plot( amo['times'], amo['rindices'], color="#FF2020", lw=1.5, label="Response AMO" )
...
def plot_ccmp_era5_timeseries_deseasonalized( ccmp_analyses, era5_analyses, outputdir, pdo=None, amo=None, enso=None, rindices=False ): 
    ...
    for ihemisphere, hemisphere in enumerate( [ "N", "S" ] ): 
        ...
        ret = expand( ccmp_rec['times'], ccmp_rec['coefficients'], ccmp_rec['uncertainty_covariance'], 
                     ccmp_rec['labels'], ccmp_rec['time_reference'], pdo=pdo, amo=amo, enso=enso, rindices=rindices )
    ...
    outputfile = os.path.join( outputdir, "ccmp_era5_deseasonalize_timeseries.pdf" )
    print( f'  Saving to {outputfile}' )
    fig.savefig( outputfile )
...
def plot_table_of_trends( ccmp_analyses, cmip6_analyses, outputdir ): 
    ...
    suffixes = [ "Oceans", "Pacific", "Atlantic" ]
    hemispheres = [ "N", "S" ]
    method = "All" 
    ...
    for ihemisphere, hemisphere in enumerate( hemispheres ): 
        for isuffix, suffix in enumerate( suffixes ): 
            ...
            for iseason, season in enumerate(seasons): 
                rec = [ rec for rec in ccmp_analyses if rec['method']==method and rec['season']==season['name'] and rec['region']==region ][0]
                x = iseason * np.array([1,1]) - 0.5 * dx
                y = rec['enso'] + rec['enso_uncertainty'] * np.array([-1,1])
                ax.plot( x, y, lw=2, color='k' )
...
def amo_trend_covariance( ccmp_analyses, cmip6_analyses, amo, season="All", outputfile="amo_trend_covariance.pdf" ): 
    ...
    print( f'amo_trend_covariance: region={region}, season={season}' )
    ...
    ax.set_ylim( -2.0, 2.0 )
    ax.set_yticks( np.arange( -2.0, 2.001, 1.0 ) )
    ax.yaxis.set_minor_locator( MultipleLocator(0.5) )
    if season == "All": 
        ax.set_ylabel( r'AMO [$^\circ$ index$^{-1}$]' )
    else: 
        ax.set_ylabel( r'AMO (' + season + r') [$^\circ$ index$^{-1}$]' )
    ...
    for imodel, model in enumerate( models ): 
        ...
        ax.plot( x * 10, y, lw=1.0, color=colors[imodel] )
...
def compare_explained_variance( outputfile, analyses, labels ): 
    ...
    print( "compare_explained_variance" )

    latex = ( outputfile[-4:] == ".tex" )
    ...
    if latex: 
        ...
        lines.append( "Region" + " "*8 + "  ".join( [ f'{season:^14s}' for season in seasons ] ) )
...
def analyze_berkeley_trends(): 

    berkeley = get_berkeley_temperature() 
    times = berkeley['years'] + ( berkeley['months'] - 0.5 ) / 12
    data = berkeley['annual_data']

    for yearrange in ( [1995,2024], [1985,2014] ): 
        ii = np.argwhere( np.logical_and( berkeley['years'] >= yearrange[0], berkeley['years'] <= yearrange[1] ) ).squeeze()
        ret = regress( times[ii], data[ii] )
        trend = ret['coefficients'][ ret['labels'].index("trend") ]
        print( f'For yearrange = {yearrange[0]}--{yearrange[1]}, trend = {trend*10:6.3f} K/dec' )
...
def execute_all(): 
    ...
    rindices = False
    outputdir = "with_amo_pdo"
    os.makedirs( outputdir, exist_ok=True )

    ccmp_analyses = compute_ccmp_linear_regressions( outputdir, pdo=pdo, amo=amo, enso=enso, rindices=rindices ) 
    ...
    if pdo is not None: 
        plot_pdo_timeseries( pdo, outputfile=os.path.join(outputdir,"pdo_timeseries.pdf") ) 
    if amo is not None: 
        plot_amo_timeseries( amo, outputfile=os.path.join(outputdir,"amo_timeseries.pdf") ) 
    plot_unull_timeseries( ccmp_analyses, outputfile=os.path.join(outputdir,"ccmp_tropicalwidth_timeseries.pdf") ) 
    plot_trends_by_season( ccmp_analyses, outputfile=os.path.join(outputdir,"ccmp_seasonal_timeseries.pdf") ) 
...
def execute_select(): 
    ...
    ccmp_analyses_linear = compute_ccmp_linear_regressions( outputdir, rindices=rindices )
    ccmp_analyses = compute_ccmp_linear_regressions( outputdir, pdo=pdo, amo=amo, enso=enso, rindices=rindices ) 
    cmip6_analyses = compute_cmip6_linear_regression( outputdir, pdo=pdo, amo=amo, enso=enso, rindices=rindices ) 
    era5_analyses = compute_era5_linear_regression( outputdir, pdo=pdo, amo=amo, enso=enso, rindices=rindices ) 
